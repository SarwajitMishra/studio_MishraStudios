...
export default function ImageToVideoPage() {
  ...
  const handleGenerate = async () => {
    ...
    setGeneratedVideoUrl(null);
    try {
      const result = await imageToVideo({ gcsUri, mimeType, prompt });
      setGeneratedVideoUrl(result.videoDataUri);
      toast({
        title: "Video Generated!",
        description: "Your new animated video is ready.",
      });
    } catch (error) {
      toast({
                ...
                <Card className="shadow-lg">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <ImageIcon className="w-6 h-6" /> Image to Video
                    </CardTitle>
                    <CardDescription>Upload an image and describe how to animate it.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="prompt-input">2. Describe Animation</Label>
                      <Input id="prompt-input" placeholder="e.g., Make the waterfall flow" value={prompt} onChange={(e) => setPrompt(e.target.value)} disabled={isLoading || isGenerating} />
                    </div>
